...
**Goal:** 强化本仓库多 Agent 工作流，确保阶段 1-7 只能按顺序执行，不能跳步、并行或重排；并强制分支安全策略：目标分支从当前分支创建、输入确认后先切到目标分支再落计划与进度、子 Agent 改动先汇总到目标分支、验收通过后仅推送相关分支到远端（不自动合并到当前分支），且禁止将当前分支作为目标分支。
...
### Task 3: Enforce branch-context-first documentation
...
- [x] **Step 1: Require switching to target branch immediately after input confirmation**

- [x] **Step 2: Block plan/progress creation before target branch checkout**

- [x] **Step 3: Require recording branch-context evidence in stage summary**
