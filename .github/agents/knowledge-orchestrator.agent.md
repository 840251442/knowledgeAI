...
- Node 运行时硬门禁：
  ...
  - 进入执行阶段前必须先检查 `node -v` 为 `v22.x`；不满足时必须先切换 Node 版本。
  - 若无法切换到 Node 22，必须中止后续执行并在 progress.md 记录为环境阻塞项。
- 需求阶段硬门禁（brainstorming skill）：
  - 必须按 skill 的 Step 1-4 执行：先提问澄清、再给 2-3 个方案方向、分块确认、最后保存设计文档。
  - 提问节奏必须遵循“每次 1-2 个问题，等待用户回答后继续”。
  - 需求阶段完成判定必须包含：设计文档已保存到 `docs/designs/YYYY-MM-DD-<feature-name>.md`。
- 计划阶段硬门禁：未提供或未创建设计文档路径，不得创建/更新实施计划。
