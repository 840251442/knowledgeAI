...
import { unpublishAdminArticle } from "@/services/admin-article.service";
...
export async function POST(
  ...
  try {
    const updated = await unpublishAdminArticle(id, { id: user.id, role: user.role });
    ...
    if (raw === "ARTICLE_ALREADY_DRAFT") {
      return apiError("文章未发布，无需下线", { status: 409, code: "ALREADY_DRAFT" });
    }
